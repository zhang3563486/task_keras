...
def load(main_args, sub_args, seed=42, test_size=.1):
    if main_args.mode == 'segmentation':
        if not os.path.isfile(os.path.join(sub_args['etc']['result_root'], 
                                        'trainset_{}.txt'.format(sub_args['task']['subtask']))):
            ...
            with open(os.path.join(sub_args['etc']['result_root'], 
                                'trainset_{}.txt'.format(sub_args['task']['subtask'])), 'w') as f:
                for t in trainset:
                    f.write(t+'\n')

            with open(os.path.join(sub_args['etc']['result_root'], 
                                'testset_{}.txt'.format(sub_args['task']['subtask'])), 'w') as f:
                for t in testset:
                    f.write(t+'\n')

        else:
            with open(os.path.join(sub_args['etc']['result_root'], 
                                'trainset_{}.txt'.format(sub_args['task']['subtask'])), 'r') as f:
                trainset = [t.rstrip() for t in f.readlines()]
            
            with open(os.path.join(sub_args['etc']['result_root'], 
                                'testset_{}.txt'.format(sub_args['task']['subtask'])), 'r') as f:
        ...
        trainset = pd.read_csv(os.path.join(sub_args['etc']['result_root'], 'trainset_{}.csv'.format(sub_args['task']['subtask'])))
        valset = pd.read_csv(os.path.join(sub_args['etc']['result_root'], 'valset_{}.csv'.format(sub_args['task']['subtask'])))
        testset = pd.read_csv(os.path.join(sub_args['etc']['result_root'], 'testset_{}.csv'.format(sub_args['task']['subtask'])))
...
class Preprocessing:
    ...
    def _only_liver(self, x, y):
        raise NotImplementedError('_only_liver method is not implemented.')
    ...
    def _apply_transform(self, x, transform_matrix, fill_mode='nearest', cval=0.):
